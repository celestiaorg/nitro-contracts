...
library MerkleTreeAccumulatorLib {
  ...
  function appendCompleteSubTree(
    ...
    require(
      level < me.length,
      'Level greater than highest level of current expansion'
    );
  ...
  function verifyPrefixProof(
    ...
    require(
      treeSize(preExpansion) == preSize,
      'Pre size does not match expansion'
    );
    ...
    bytes32[] memory exp = ArrayUtilsLib.slice(
      preExpansion,
      0,
      preExpansion.length
    );
