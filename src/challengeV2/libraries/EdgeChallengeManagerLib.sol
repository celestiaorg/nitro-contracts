...
library EdgeChallengeManagerLib {
  ...
  function layerZeroTypeSpecificChecks(
    ...
    if (
      ChallengeEdgeLib.levelToType(args.level, numBigStepLevel) ==
      EdgeType.Block
    ) {
      ...
      (bytes32[] memory inclusionProof, , ) = abi.decode(
        args.proof,
        (bytes32[], AssertionStateData, AssertionStateData)
      );
      ...
      bytes32 startStateHash = oneStepProofEntry.getMachineHash(
        ard.startState.toExecutionState()
      );
      bytes32 endStateHash = oneStepProofEntry.getMachineHash(
        ard.endState.toExecutionState()
      );

      return (
        ProofData(startStateHash, endStateHash, inclusionProof),
        originId
      );
    } else {
  ...
  function createLayerZeroEdge(
    ...
    bytes32 startHistoryRoot = layerZeroCommonChecks(
      proofData,
      args,
      expectedEndHeight
    );
  ...
  function timeUnrivaledTotal(
    ...
    if (store.edges[edgeId].lowerChildId != bytes32(0)) {
      uint256 lowerTimer = store
        .edges[store.edges[edgeId].lowerChildId]
        .totalTimeUnrivaledCache;
      uint256 upperTimer = store
        .edges[store.edges[edgeId].upperChildId]
        .totalTimeUnrivaledCache;
  ...
  function updateTimerCache(
    ...
    uint256 currentAccuTimer = validateCurrentTimer(
      store,
      edgeId,
      maximumCachedTime
    );
  ...
  function updateTimerCacheByChildren(
    ...
    return
      updateTimerCache(
        store,
        edgeId,
        timeUnrivaledTotal(store, edgeId),
        maximumCachedTime
      );
  }
  ...
  function updateTimerCacheByClaim(
    ...
    return
      updateTimerCache(store, edgeId, totalTimeUnrivaled, maximumCachedTime);
  }
  ...
  function bisectEdge(
    ...
    uint256 middleHeight = mandatoryBisectionHeight(
      ce.startHeight,
      ce.endHeight
    );
    {
  ...
  function confirmEdgeByTime(
    ...
    if (totalTimeUnrivaled < confirmationThresholdBlock) {
      revert InsufficientConfirmationBlocks(
        totalTimeUnrivaled,
  ...
  function confirmEdgeByOneStepProof(
    ...
    if (
      ChallengeEdgeLib.levelToType(
        store.edges[edgeId].level,
        numBigStepLevel
      ) != EdgeType.SmallStep
    ) {
