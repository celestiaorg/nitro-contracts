...
contract EdgeChallengeManager is IEdgeChallengeManager, Initializable {
  ...
  function createLayerZeroEdge(
    ...
    EdgeType eType = ChallengeEdgeLib.levelToType(
      args.level,
      NUM_BIGSTEP_LEVEL
    );
    ...
    if (address(st) != address(0) && sa != 0) {
      ...
      address receiver = edgeAdded.hasRival
        ? excessStakeReceiver
        : address(this);
  ...
  function updateTimerCacheByChildren(
    ...
    (bool updated, uint256 newValue) = store.updateTimerCacheByChildren(
      edgeId,
      maximumCachedTime
    );
  ...
  function confirmEdgeByTime(
    ...
    // the this edge
    bool isBlockLevel = ChallengeEdgeLib.levelToType(
      topEdge.level,
      NUM_BIGSTEP_LEVEL
    ) == EdgeType.Block;
    if (isBlockLevel && assertionChain.isFirstChild(topEdge.claimId)) {
      ...
      assertionBlocks =
        ...
        assertionChain.getFirstChildCreationBlock(
          claimStateData.prevAssertionHash
        );
    }

    uint256 totalTimeUnrivaled = store.confirmEdgeByTime(
      edgeId,
      assertionBlocks,
      challengePeriodBlocks
    );

    emit EdgeConfirmedByTime(
      edgeId,
