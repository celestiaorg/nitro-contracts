...
contract MockOneStepProofEntry is IOneStepProofEntry {
  ...
  function getStartMachineHash(
    ...
    return
      keccak256(abi.encodePacked('Machine:', globalStateHash, wasmModuleRoot));
  }
  ...
  function proveOneStep(
    ...
    if (testMachineStep != 0)
      require(testMachineStep == machineStep, 'Invalid machine step');
...
contract EdgeChallengeManagerLibTest is Test {
  ...
  function twoNonRivals()
    internal
    returns (ChallengeEdge memory, ChallengeEdge memory)
  {
    bytes32 originId = rand.hash();

    ChallengeEdge memory edge1 = ChallengeEdgeLib.newChildEdge(
      originId,
      rand.hash(),
      3,
      rand.hash(),
    ...
    ChallengeEdge memory edge2 = ChallengeEdgeLib.newChildEdge(
      originId,
      rand.hash(),
      3,
      rand.hash(),
  ...
  function testAddRivals() public {
    ...
    assertTrue(
      store.firstRivals(se2.mutualIdMem()) == edge2.idMem(),
      'First rival1'
    );
    assertTrue(
      store.firstRivals(se.mutualIdMem()) == edge2.idMem(),
      'First rival2'
    );
  ...
  function testAddMoreRivals() public {
    ...
    assertTrue(
      store.firstRivals(se3.mutualIdMem()) == edge2.idMem(),
      'First rival3'
    );
  }
  ...
  function testCannotAddSameEdgeTwice() public {
    ChallengeEdge memory edge = ChallengeEdgeLib.newChildEdge(
      rand.hash(),
      rand.hash(),
      0,
      rand.hash(),
    ...
    vm.expectRevert(
      abi.encodeWithSelector(EdgeAlreadyExists.selector, edge.idMem())
    );
    store.add(edge);
  }
  ...
  function testTimeUnrivaledFirstRivalEdgeNotExist() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(EdgeNotExists.selector, edge2.idMem())
    );
  ...
  function testTimeUnrivaledAfterRival() public {
    ...
    ChallengeEdge memory edge4 = ChallengeEdgeLib.newChildEdge(
      originId,
      rand.hash(),
      3,
      rand.hash(),
  ...
  function testMandatoryBisectHeightSizes() public {
    ...
    assertEq(
      store.mandatoryBisectionHeight(765273563, 10898783768364),
      8796093022208
    );
  }
  ...
  function getExpansion(uint256 leafCount) internal returns (bytes32[] memory) {
    ...
    bytes32[] memory expansion = ProofUtils.expansionFromLeaves(
      hashes,
      0,
      leafCount
    );
  ...
  function appendRandomStatesBetween(
    ...
    (bytes32[] memory states, bytes32[] memory exp) = appendRandomStates(
      currentStates,
      numStates - 1
    );
    bytes32[] memory fullStates = ArrayUtilsLib.append(states, endState);
    bytes32[] memory fullExp = MerkleTreeAccumulatorLib.appendLeaf(
      exp,
      endState
    );
  ...
  function rivalStates(
    ...
    (bytes32[] memory agreeStates, ) = appendRandomStates(
      preStates,
      agreePoint - start
    );
    (bytes32[] memory states1, ) = appendRandomStates(
      agreeStates,
      end - agreePoint
    );

    (bytes32[] memory states2, ) = appendRandomStates(
      agreeStates,
      end - agreePoint
    );
  ...
  function edgeFromStates(
    ...
    bytes32 startRoot = MerkleTreeAccumulatorLib.root(
      ProofUtils.expansionFromLeaves(states, 0, start + 1)
    );
    bytes32 endRoot = MerkleTreeAccumulatorLib.root(
      ProofUtils.expansionFromLeaves(states, 0, end + 1)
    );
  ...
  function proofGen(
    ...
    return
      ProofUtils.generatePrefixProof(
        start,
        ArrayUtilsLib.slice(states, start, states.length)
      );
  }
  ...
  function twoRivalsFromLeaves(
    ...
    internal
    returns (
      ChallengeEdge memory,
      ChallengeEdge memory,
      bytes32[] memory,
      bytes32[] memory
    )
  {
    (bytes32[] memory states1, bytes32[] memory states2) = rivalStates(
      start,
      agreePoint,
      end
    );

    ChallengeEdge memory edge1 = edgeFromStates(
      rand.hash(),
      start,
      end,
      states1
    );
    ChallengeEdge memory edge2 = edgeFromStates(
      edge1.originId,
      start,
      end,
      states2
  ...
  function bisectAndCheck(
    ...
    bytes memory prefixProof = abi.encode(
      ...
      ProofUtils.generatePrefixProof(
        bisectionPoint + 1,
        ArrayUtilsLib.slice(states, bisectionPoint + 1, states.length)
      )
    );
    (
      bytes32 lowerChildId,
  ...
  function testBisectEdge() public {
    ...
    assertFalse(
      store.hasRival(store.get(edge1.idMem()).upperChildId),
      'Upper child rival'
    );
    ...
    assertFalse(
      store.hasRival(store.get(edge2.idMem()).upperChildId),
      'Upper child rival'
    );
  }
  ...
  function testBisectNoRival() public {
    ...
    bytes memory proof = abi.encode(
      ...
      ProofUtils.generatePrefixProof(
        bisectionPoint + 1,
        ArrayUtilsLib.slice(states1, bisectionPoint + 1, states1.length)
      )
    );
  ...
  function testBisectTwice() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        EdgeAlreadyExists.selector,
        store.get(edgeId).upperChildId
      )
    );
    store.bisectEdge(edgeId, bisectionRoot1, proof);
  }
  ...
  function testBisectEdgeLengthOne() public {
    ...
    bytes32 edgeId = edge1.idMem();
    vm.expectRevert(
      abi.encodeWithSelector(HeightDiffLtTwo.selector, start, end)
    );
  ...
  function bisectArgs(
    ...
    bytes memory proof = abi.encode(
      ...
      ProofUtils.generatePrefixProof(
        bisectionPoint + 1,
        ArrayUtilsLib.slice(states, bisectionPoint + 1, end + 1)
      )
    );
  ...
  function addParentAndChildren(
    ...
    (, bytes32 bisectionRoot, bytes memory bisectionProof) = bisectArgs(
      states1,
      start,
      end
    );

    (bytes32 lowerChildId, , EdgeAddedData memory upperChildAdded) = store
      .bisectEdge(edge1.idMem(), bisectionRoot, bisectionProof);
  ...
  function testNextlevel() public {
    ...
    assertTrue(
      store.nextEdgeLevel(NUM_BIGSTEP_LEVEL, NUM_BIGSTEP_LEVEL) ==
        NUM_BIGSTEP_LEVEL + 1
    );
    vm.expectRevert(
      abi.encodeWithSelector(
        LevelTooHigh.selector,
        NUM_BIGSTEP_LEVEL + 2,
        NUM_BIGSTEP_LEVEL
      )
    );
  ...
  function bisect(
    ...
    (bytes32 lowerChildId, , EdgeAddedData memory upperChildAdded) = store
      .bisectEdge(edge.idMem(), bisectionRoot, bisectionProof);
  ...
  function addParentsAndChildren(
    ...
    (bytes32 lowerChildId1, bytes32 upperChildId1) = bisect(
      edge1,
      states1,
      start,
      end
    );
    (bytes32 lowerChildId2, bytes32 upperChildId2) = bisect(
      edge2,
      states2,
      start,
      end
  ...
  function testTimeUnrivaledTotalAndUpdateTimerCacheByChildren() public {
    ...
    // bisect first parent
    (, bytes32 bisectionRoot1, bytes memory bisectionProof1) = bisectArgs(
      states1,
      2,
      8
    );
    (bytes32 lowerChildId1, , EdgeAddedData memory upperChildAdded1) = store
      .bisectEdge(edge1.idMem(), bisectionRoot1, bisectionProof1);

    // roll forward a bit
    vm.roll(block.number + 20);

    // bisect second parent
    (, bytes32 bisectionRoot2, bytes memory bisectionProof2) = bisectArgs(
      states2,
      2,
      8
    );
    (bytes32 lowerChildId2, , EdgeAddedData memory upperChildAdded2) = store
      .bisectEdge(edge2.idMem(), bisectionRoot2, bisectionProof2);
    ...
    vm.expectRevert(
      abi.encodeWithSelector(CachedTimeSufficient.selector, 220, 220)
    );
    store.updateTimerCacheByChildren(lowerChildId2, 220);
    vm.expectRevert(
      abi.encodeWithSelector(CachedTimeSufficient.selector, 0, 0)
    );
  ...
  function addUpLastLevel()
    internal
    returns (bytes32 originId, uint256[] memory startHeights)
  {
    originId = rand.hash();
  ...
  function confirmByOneStep(uint256 flag) internal {
    ...
    data.beforeProof = ProofUtils.generateInclusionProof(
      ProofUtils.rehashed(ArrayUtilsLib.slice(states1, 0, startHeight + 1)),
      startHeight
    );
    if (flag == 6) {
    ...
    data.afterProof = ProofUtils.generateInclusionProof(
      ProofUtils.rehashed(states1),
      startHeight + 1
    );
    if (flag == 7) {
    ...
    if (bytes(data.revertArg).length != 0) {
      ...
      assertTrue(
        store.get(eid).status == EdgeStatus.Confirmed,
        'Edge confirmed'
      );
      assertEq(
  ...
  error MachineStep(
    uint256 actual,
    uint256 expected,
    uint256[] startHeights,
    uint256 smallStartHeight
  );
  ...
  function newRootsAndProofs(
    ...
    bytes32[] memory states;
    {
      ...
      states = ArrayUtilsLib.concat(
        ArrayUtilsLib.concat(startStates, innerStates),
        endStates
      );
    }
    bytes32[] memory startExp = ProofUtils.expansionFromLeaves(
      states,
      0,
      startHeight + 1
    );
    ...
    bytes32[] memory startInclusionProof = ProofUtils.generateInclusionProof(
      ProofUtils.rehashed(ArrayUtilsLib.slice(states, 0, startHeight + 1)),
      startHeight
    );
    bytes32[] memory endInclusionProof = ProofUtils.generateInclusionProof(
      ProofUtils.rehashed(states),
      endHeight
    );

    bytes32[] memory prefixProof = ProofUtils.generatePrefixProof(
      startHeight + 1,
      ArrayUtilsLib.slice(states, startHeight + 1, endHeight + 1)
    );
  ...
  function createZeroBlockEdge(
    ...
    if (mode == 151) {
      ...
      revertArg = abi.encodeWithSelector(
        AccountHasMadeLayerZeroRival.selector,
        address(this),
        expectedMutualId
      );
    }
    ...
    if (mode != 144) {
      ...
      if (mode == 141) {
        ...
        revertArg = abi.encodeWithSelector(
          AssertionHashMismatch.selector,
          ard.assertionHash,
          claimId
        );
      }
  ...
  function createSmallStepEdge(uint256 mode) internal {
    ...
    (vars.claimId, vars.claimRoots) = createClaimEdge(
      store,
      vars.claimStartHeight,
      vars.claimEndHeight,
      mode == 160 ? false : true
    );
    if (mode == 160) {
      vars.revertArg = abi.encodeWithSelector(
        ClaimEdgeNotLengthOneRival.selector,
        vars.claimId
      );
    }
    if (mode == 161) {
    ...
    if (mode == 164) {
      ...
      b[0] = rand.hash();
      vars.claimRoots.startInclusionProof = ArrayUtilsLib.concat(
    ...
    if (mode == 165) {
      bytes32[] memory b = new bytes32[](1);
      b[0] = rand.hash();
      vars.claimRoots.endInclusionProof = ArrayUtilsLib.concat(
    ...
    if (mode == 163) {
      vars.revertArg = abi.encodeWithSelector(
        ClaimEdgeInvalidLevel.selector,
        NUM_BIGSTEP_LEVEL,
        NUM_BIGSTEP_LEVEL
      );
    }
  ...
  AssertionStateData genesisStateData =
    AssertionStateData(genesisState, bytes32(0), bytes32(0));
  ...
  function createLayerZeroEdge(
    ...
    bytes memory prefixProof = abi.encode(
      ...
      ProofUtils.generatePrefixProof(
        1,
        ArrayUtilsLib.slice(states, 1, states.length)
      )
    );
  ...
  function bisect(
    ...
    bytes32[] memory middleExp = ProofUtils.expansionFromLeaves(
      states,
      0,
      bisectionSize + 1
    );
    bytes32[] memory upperStates = ArrayUtilsLib.slice(
      states,
      bisectionSize + 1,
      endSize + 1
    );

    (bytes32 lowerChildId, , EdgeAddedData memory upperChild) = store
      .bisectEdge(
        edgeId,
        MerkleTreeAccumulatorLib.root(middleExp),
        abi.encode(
          middleExp,
          ProofUtils.generatePrefixProof(bisectionSize + 1, upperStates)
        )
      );
    ...
    returns (BisectionChildren[6] memory, BisectionChildren[6] memory)
  {
    ...
    winningEdges[3] = bisect(
      winningEdges[4].lowerChildId,
      args.winningLeaves,
      8,
      16
    );
    losingEdges[3] = bisect(
      losingEdges[4].lowerChildId,
      args.losingLeaves,
      8,
      16
    );
    ...
    // height 4
    winningEdges[2] = bisect(
      winningEdges[3].lowerChildId,
      args.winningLeaves,
      4,
      8
    );
    losingEdges[2] = bisect(
      losingEdges[3].lowerChildId,
      args.losingLeaves,
      4,
      8
    );

    winningEdges[1] = bisect(
      winningEdges[2].lowerChildId,
      args.winningLeaves,
      2,
      4
    );
    losingEdges[1] = bisect(
      losingEdges[2].lowerChildId,
      args.losingLeaves,
      2,
      4
    );
    ...
    // height 2
    winningEdges[0] = bisect(
      winningEdges[1].lowerChildId,
      args.winningLeaves,
      1,
      2
    );
    if (!args.skipLast) {
      losingEdges[0] = bisect(
        losingEdges[1].lowerChildId,
        args.losingLeaves,
        1,
        2
      );
    ...
    )
  {
    ...
    bytes32 edge1Id;
    {
      ...
      (states1, exp1) = appendRandomStatesBetween(
        genesisStates(),
        StateToolsLib.mockMachineHash(args.endState1),
        height1
      );
    ...
    bytes32 edge2Id;
    {
      ...
      (states2, exp2) = appendRandomStatesBetween(
        genesisStates(),
        StateToolsLib.mockMachineHash(args.endState2),
        height1
      );
  ...
  function createMachineEdgesAndBisectToFork(
    ...
    bytes32 edge1Id;
    {
      ...
      {
        bytes32[] memory claimStartInclusionProof = ProofUtils
          .generateInclusionProof(
            ProofUtils.rehashed(ArrayUtilsLib.slice(args.forkStates1, 0, 1)),
            0
          );
        bytes32[] memory claimEndInclusionProof = ProofUtils
          .generateInclusionProof(ProofUtils.rehashed(args.forkStates1), 1);
        ...
        .createLayerZeroEdge(
          CreateEdgeArgs({
            ...
            prefixProof: abi.encode(
              ...
              ProofUtils.generatePrefixProof(
                1,
                ArrayUtilsLib.slice(states1, 1, states1.length)
              )
            ),
            proof: typeSpecificProof1
          }),
    ...
    bytes32 edge2Id;
    {
      bytes memory typeSpecificProof2;
      {
        bytes32[] memory claimStartInclusionProof = ProofUtils
          .generateInclusionProof(
            ProofUtils.rehashed(ArrayUtilsLib.slice(args.forkStates2, 0, 1)),
            0
          );
        bytes32[] memory claimEndInclusionProof = ProofUtils
          .generateInclusionProof(ProofUtils.rehashed(args.forkStates2), 1);
        bytes32[] memory edgeInclusionProof = ProofUtils.generateInclusionProof(
          ProofUtils.rehashed(states2),
          states2.length - 1
        );
        ...
        .createLayerZeroEdge(
          CreateEdgeArgs({
            ...
            prefixProof: abi.encode(
              ...
              ProofUtils.generatePrefixProof(
                1,
                ArrayUtilsLib.slice(states2, 1, states2.length)
              )
            ),
