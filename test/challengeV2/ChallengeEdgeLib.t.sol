...
contract ChallengeEdgeLibAccess {
  ...
  function setChildren(bytes32 lowerChildId, bytes32 upperChildId) public {
    return
      ChallengeEdgeLib.setChildren(storageEdge, lowerChildId, upperChildId);
  }
...
contract ChallengeEdgeLibTest is Test {
  ...
  function testLength() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(EdgeNotExists.selector, layerZero.id())
    );
    layerZero.length();
  ...
  function testSetChildren() public {
    ...
    assertEq(
      child.getChallengeEdge().lowerChildId,
      lowerChildId,
      'Lower child id'
    );
    assertEq(
      child.getChallengeEdge().upperChildId,
      upperChildId,
      'Upper child id'
    );
  ...
  function testSetConfirmed() public {
    ...
    child.setConfirmed();
    assertTrue(
      child.getChallengeEdge().status == EdgeStatus.Confirmed,
      'Status confirmed'
    );
    assertTrue(
      child.getChallengeEdge().confirmedAtBlock == 137,
      'Confirmed at block'
    );
  }
  ...
  function testLevelToType() public {
    ...
    assertTrue(
      access.levelToType(1, numBigStep) == EdgeType.BigStep,
      'Big step 1'
    );
    assertTrue(
      access.levelToType(2, numBigStep) == EdgeType.BigStep,
      'Big step 2'
    );
    assertTrue(
      access.levelToType(3, numBigStep) == EdgeType.BigStep,
      'Big step 3'
    );
    assertTrue(
      access.levelToType(4, numBigStep) == EdgeType.BigStep,
      'Big step 4'
    );
    assertTrue(
      access.levelToType(5, numBigStep) == EdgeType.SmallStep,
      'Small step'
    );
