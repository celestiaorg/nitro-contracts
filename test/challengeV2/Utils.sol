...
contract Random {
  bytes32 private seed =
    0xf19f64ef5b8c788ff3f087b4f75bc6596a6aaa3c9048bbbbe990fa0870261385;
...
library ProofUtils {
  ...
  function expansionFromLeaves(
    ...
    require(
      leafEndIndex <= leaves.length,
      'Leaf end not less than leaf length'
    );
  ...
  function generatePrefixProof(
    ...
    while (size < postSize) {
      uint256 level = MerkleTreeAccumulatorLib.maximumAppendBetween(
        size,
        postSize
      );
      ...
      bytes32[] memory exp = expansionFromLeaves(
        newLeaves,
        startIndex,
        endIndex
      );
    ...
    while (prevLayer.length > 1) {
      ...
      for (uint256 i = 0; i < nextLayer.length; i++) {
        if (2 * i + 1 < prevLayer.length) {
          ...
          nextLayer[i] = keccak256(
            abi.encodePacked(prevLayer[2 * i], bytes32(0))
          );
        }
      }
      layers[l] = nextLayer;
